...
shinyServer(function(input, output) {
    ...
    rv <- reactiveValues(Chou_HC = data.frame(), 
                         HC_dt = data.frame(DOL = numeric(0), CGA = numeric(0), HC = numeric(0), Z = numeric(0), Percentile = numeric(0)),
                         offset = NA,
                         percentile_min = NA
    )
    ...
    observeEvent(input$plot, {
        ...
        rv$HC_dt <- data.frame(DOL = numeric(0), CGA = numeric(0), HC = numeric(0), Z = numeric(0), Percentile = numeric(0))
        
        output$msg <- renderUI({})
        
        if (is.na(GAn)) {
            output$msg <- renderUI({
                div(style="color:#cc0000;font-weight:500;font-size:16px;", "Please enter GA!")
            })
            ...
            if (input$sex == "") {
                ...
                output$table <- renderUI({
                    fluidRow(
                        div(DTOutput("hc_table"), style = "height:360px;"),
                        div(style="padding:6px 0 0 0;", 
                            column(10, uiOutput("msg"), style="padding-left:0px;"),
                            column(2, align="right", style="padding-right:0px;", disabled(actionButton("calculate", "PLOT", style="padding:3px 6px 3px 6px;font-size:14px;background-color:#546E7A;color:#FFFFFF;border:0;")))
                        )
                    )
                })
                
                output$hc_table <- renderDT({
                    datatable(rv$HC_dt %>% select(DOL, Week, Day, HC, Z) %>% rename(`Z (%ile)` ="Z"), 
                              rownames = FALSE, 
                              selection = "single",
                ...
                output$HC_plot <- renderPlot({
                    ...
                    ggplot(rv$Chou_HC, aes(x = ((Day + rv$offset)/7))) +
                        ...
                        geom_point(data = rv$HC_dt %>% filter(!is.na(HC)), aes(x = PMA/7, y = HC), color = "#CC0000", size = 3) +
                        geom_line(data = rv$Chou_HC, 
    ...
    observeEvent(input$hc_table_cell_edit, {
        info <- input$hc_table_cell_edit
        if (info$col == 3) {
            ...
            if (is.na(as.numeric(info$value))) {
                ...
                Z <- (as.numeric(info$value) - expected_HC) / expected_sigma
                Percentile <- 100 * pnorm(Z)
                rv$proxy[info$row, "Z"] <- paste0(round(digits = 2, Z), " (", round(digits = 1, Percentile), ")")
                
                percentile_df <- rv$proxy %>% left_join(rv$Chou_HC %>% select(Day, Predicted_expected, sigma) %>% mutate(PMA = Day + rv$offset) %>% select(-Day))
