...
shinyServer(function(input, output) {
    ...
    observeEvent(input$submit, {
        ...
        if (rv$registrantType + rv$firstName + rv$lastName + rv$gmail + rv$username + rv$pwd == 0) {
            ...
            write_csv(data.frame(id = input$username, color_theme = 1), "www/user.csv", append = TRUE)
            ...
            output$message <- renderUI({
                fluidRow(
                    column(3,
                           div(align="right", style="font-size:120px;color:#F1C232;",span(icon("envelope")))),
                    column(9,
                           div(style="color:#54617a;font-size:16px;font-weight:500;padding:36px 12px 12px 12px;", 
                               ...
                               div("Please click", a("HERE", href="https://sites.google.com/view/nicugrowth-app-full/home", target = "_blank"), "to be re-directed to the full-functioning WebApp", style="padding-top:6px;"), 
                               #div("Once approved, a link to the full-functioning WebApp will be E-Mailed to you.")
                               ))
                )
