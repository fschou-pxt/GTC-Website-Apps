plot_by_DOL <- function(table, type, colors = list(), font = "Nunito") {
  ...
  ggplot(GTC_dt, aes(x = (DOL/7))) +
    geom_line(aes(y = percentile_3_var * factor), size = 0.75, color = colors$color3, linetype = "solid") +
    geom_line(aes(y = percentile_10_var * factor), size = 0.5, color = colors$color3, linetype = "dashed") +
    geom_line(aes(y = percentile_25_var * factor), size = 0.5, color = colors$color3, linetype = "dotted") +
    geom_line(aes(y = percentile_50_var * factor), size = 0.5, color = colors$color3, linetype = "solid") +
    geom_line(aes(y = percentile_75_var * factor), size = 0.5, color = colors$color3, linetype = "dotted") +
    geom_line(aes(y = percentile_90_var * factor), size = 0.5, color = colors$color3, linetype = "dashed") +
    geom_line(aes(y = percentile_97_var * factor), size = 0.75, color = colors$color3, linetype = "solid") +
    geom_point(data = table, mapping = aes(y = value, color = `_Group`), size = 2) +
    geom_line(data = table, mapping = aes(y = value, group = ID, color = `_Group`), size = 0.75) +
    labs(x = "\nWeek of life", y = y_title, title = NULL) +
    scale_x_continuous(breaks = 2* c(0:100)) +
    theme_bw() +
