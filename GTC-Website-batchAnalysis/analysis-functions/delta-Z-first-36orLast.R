delta_Z_first_36orLast <- function(table, type, colors = list(), font) {
  Z <- table %>%
    group_by(ID) %>%
    summarise(
  ...
  Z <- table %>%
    ...
    mutate(PMA = GA_week * 7 + GA_day + DOL) %>%
    filter(PMA <= 252) %>%
    rename(value = paste0("_", type, " Z")) %>%
    group_by(ID) %>%
    summarise(
      Group = unique(`_Group`),
      `first Z` = value[DOL == min(DOL)],
      `last Z` = value[DOL == max(DOL)],
      `PMA36wk Z` = ifelse(252 %in% PMA, value[PMA == 252], NA)
    ) %>%
    ungroup(.) %>%
    mutate(`delta Z` = ifelse(is.na(`PMA36wk Z`), `last Z`, `PMA36wk Z`) - `first Z`)
  
  plot <- 
    ggplot(Z, aes(x = Group, y = `delta Z`)) +
    geom_boxplot(aes(group = Group)) +
    geom_point(aes(color = Group)) +
    labs(x = "\nGroup", y = "Delta Z Score (PMA36wk or earlier - First)", title = "Delta Z Score (PMA 36wk or Last if earlier - First) by Group") +
    theme_bw() +
  ...
  summary <- Z %>%
    ...
    summarise(
      `Number of infants` = n(),
      `mean ± sd` = paste0(round(digits = 1, mean(`delta Z`, na.rm = T)), " ± ", round(digits = 1, sd(`delta Z`, na.rm = T))),
      `median (IQR)` = paste0(round(digits = 1, median(`delta Z`, na.rm = T)), " (", paste0(collapse = ", ", round(digits = 1, quantile(`delta Z`, probs = c(0.25, 0.75), na.rm = T))) , ")"),
